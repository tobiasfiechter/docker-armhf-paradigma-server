...
sub Process
{
  ...
  SendMail("Fusspunkt ge�ndert ($FusspunktOld -> $Fusspunkt)") if ($FusspunktOld && $Fusspunkt != $FusspunktOld);
  SendMail("Steilheit ge�ndert ($SteilheitOld -> $Steilheit)") if ($SteilheitOld && $Steilheit != $SteilheitOld);
  #XXX
  open(F, ">>x.data");
  printf(F "%02d:%02d ", (localtime)[2], (localtime)[1]);
  for (my $i = 0; $i < $#n; $i++) {
...
#XXX range f�r jedes extra (relais nur 0/1 etc)
